...
void main() {
  ...
  float noisefactor = random(position, uTime) * 0.05 + 0.05;
  float offsetX = sin(uTime + angle * 3.0) * noisefactor;
  float offsetY = cos(uTime + angle * 2.0) * noisefactor;

  animatedPosition.x = (radius + offsetX) * cos(angle + uTime) * cos(uTime);
  animatedPosition.y = (radius + offsetY) * sin(angle + uTime) * sin(uTime);
