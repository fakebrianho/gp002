...
void main() {
  ...
  float noiseFactor = random(position, time) * 0.1 + 0.05;
  float offsetX = sin(time + angle * 3.0) * noiseFactor;
  float offsetY = cos(time + angle * 2.0) * noiseFactor;

  animatedPosition.x += offsetX;
