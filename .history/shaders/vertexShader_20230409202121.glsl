...
------------------------------*/
  ...
  float individualSpeed = random(vec3(12.9898, 78.233, 37.719), 1.0) * 0.75 + 0.75;

  // Apply the circular motion and FBM offset
  animatedPosition.x = (radius + offsetX) * cos(angle + uTime * individualSpeed);
  animatedPosition.y = (radius + offsetY) * sin(angle + uTime * individualSpeed);
