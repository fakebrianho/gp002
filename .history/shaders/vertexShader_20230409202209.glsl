...
------------------------------*/
  ...
  // Calculate individual rotation speed for each particle
  float individualSpeed = random(vec3(12.9898, 78.233, 37.719), 1.0) * 0.5 + 0.5;

  // Apply the circular motion and FBM offset
  animatedPosition.x = (radius + offsetX) * cos(angle + uTime * individualSpeed * 0.75);
  animatedPosition.y = (radius + offsetY) * sin(angle + uTime * individualSpeed * 0.75);
  ...
  gl_PointSize = mix(1.0, 8.0, normalizedDistance);
